...
use crate::{pam_syslog, D, IF_NO_UTPAMH, UTPAM_FROM_MODULE};

//管理密码或认证令牌的变更
pub fn utpam_chauthtok(utpamh: &mut Option<Box<UtpamHandle>>, flags: u32) -> u8 {
    D!("called");
    ...
    if retval != PAM_INCOMPLETE {
        utpam_sanitize(utpamh);
        utpamh.former.update = UtpamBoolean::UtpamFalse;
        D!("exiting {:?} - {:?}", retval, utpamh.former.choice);
    } else {
