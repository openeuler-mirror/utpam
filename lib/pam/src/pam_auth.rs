...
// C兼容的认证函数
#[no_mangle]
pub extern "C" fn pam_authenticate(pamh: pam_handle_t, flags: c_int) -> c_int {
    ...
    utpam_authenticate(utpamh, flags as u32) as c_int
}

// C兼容的设置凭证函数
#[no_mangle]
pub extern "C" fn pam_setcred(pamh: pam_handle_t, flags: c_int) -> c_int {
    if pamh.data.is_null() {
        return PAM_SYSTEM_ERR as c_int;
    }
    let utpamh: &mut Option<Box<UtpamHandle>> =
    ...
    utpam_setcred(utpamh, flags as u32) as c_int
}
