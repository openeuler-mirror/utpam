...
// C兼容的pam_start_confdir函数
#[no_mangle]
pub extern "C" fn pam_start_confdir(
    service_name: *const c_char,
    user: *const c_char,
    pam_conversation: *const PamConv,
    ...
    if pamh.is_null() {
        log::debug!(
            "{} {}",
            LOG_ERR,
            "pam_start_confdir: invalid argument: pamh == NULL"
        );
        return PAM_SYSTEM_ERR as c_int;
    }
    let service_name = if service_name.is_null() {
        log::debug!(
            "{} {}",
            LOG_ERR,
            "pam_start_confdir: invalid argument: service == NULL"
        );
        return PAM_SYSTEM_ERR as c_int;
    } else {
        unsafe { CStr::from_ptr(service_name).to_string_lossy().into_owned() }
    ...
    if pam_conversation.is_null() {
        log::debug!(
            "{} {}",
            LOG_ERR,
            "pam_start_confdir: invalid argument: conv == NULL"
        );
        return PAM_SYSTEM_ERR as c_int;
    }
    ...
    let confdir = if confdir.is_null() {
        ...
        let dir = unsafe { CStr::from_ptr(confdir).to_string_lossy().into_owned() };
        PathBuf::from(dir)
    };

    let conv = pamconv_to_utpamconv(pam_conversation);
    let mut utpamh: Option<Box<UtpamHandle>> = None;
    let result = utpam_start_confdir(
        service_name,
        user,
        Some(Rc::new(conv)),
        confdir,
